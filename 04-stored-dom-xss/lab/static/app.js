...
(function () {
  ...
  // ── Vulnerable stored-review renderer (DOM-based / client-side stored) ──────
  // THIS is the lab's real vulnerability. The board fetches the stored reviews
  // as JSON from /api/reviews and writes each review's BODY into the page with
  // innerHTML. The server only returns data; the unsafe HTML parsing happens
  // here, in the browser - so a payload stored once fires for every viewer.
  //
  // Note: a <script> inserted via innerHTML does NOT execute (HTML5), so a
  // stored exploit uses an event-handler payload such as <img src=x onerror=...>.
  var reviewsEl = document.getElementById("reviews");
  var reviewsCount = document.getElementById("reviews-count");

  function escText(s) {
    // name and timestamp are shown safely; only the body is the unsafe sink.
    return String(s)
  ...
  function renderReviews() {
    ...
    fetch("/api/reviews")
      ...
      .then(function (reviews) {
        ...
        reviewsEl.innerHTML = reviews.map(function (rev) {
          return '<div class="card" style="margin-bottom:10px;">' +
                   '<div style="font-size:.8rem;color:#8b949e;margin-bottom:4px;">' +
                     '<strong style="color:#e6edf3;">' + escText(rev.name) +
                     '</strong> · ' + escText(rev.ts) +
                   '</div>' +
                   // === VULNERABILITY: a STORED body written to an innerHTML sink
                   '<div>' + rev.body + '</div>' +
                   // ====================================================================
                 '</div>';
        }).join("");
      ...
      .catch(function () {
        if (reviewsEl) reviewsEl.textContent = "Could not load reviews.";
      });
  }
